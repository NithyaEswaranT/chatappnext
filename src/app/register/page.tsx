 ...
 * ----------------------------------
 * Handles the multi-field registration form with client-side validation
 * feedback and server-side registration via Server Action.
 */
export default function RegisterPage() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    logger.info("RegisterPage (Client)", "Registration form submitted");

    startTransition(async () => {
      const result = await registerAction(formData);
      if (result?.error) {
        logger.warn("RegisterPage (Client)", `Registration error: ${result.error}`);
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-6 py-12">
      {/* Ambient glow blobs */}
      <div className="fixed top-0 right-1/4 w-96 h-96 bg-violet-600/10 rounded-full filter blur-3xl pointer-events-none" />
      <div className="fixed bottom-0 left-1/4 w-96 h-96 bg-indigo-600/10 rounded-full filter blur-3xl pointer-events-none" />

      <div className="glass-panel w-full max-w-md p-8 flex flex-col gap-6 shadow-2xl relative overflow-hidden">
        {/* Inner glow */}
        <div className="absolute -top-12 -left-12 w-32 h-32 rounded-full bg-indigo-600/20 filter blur-2xl" />
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {/* Confirm Password */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="confirmPassword" className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">
              Confirm Password
            </label>
            <input
              id="confirmPassword"
