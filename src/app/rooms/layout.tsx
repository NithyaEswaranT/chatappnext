...
export default async function RoomsLayout({
  ...
  return (
    <div className="flex-1 flex min-h-screen overflow-hidden">
      ...
      <aside className="w-[300px] border-r border-white/5 bg-black/40 backdrop-blur-md flex flex-col justify-between shrink-0">
        <div>
          {/* Header */}
          ...
          */}
          ...
          {/* Rooms List */}
          <div className="px-3 py-2 flex flex-col gap-1 overflow-y-auto max-h-[calc(100vh-220px)]">
            ...
            {rooms.map((room) => (
              <Link
                ...
                href={`/rooms/${room._id}`}
                className="group flex flex-col gap-0.5 px-4 py-3 rounded-lg hover:bg-white/5 border border-transparent hover:border-white/5 transition-all"
              >
      ...
      <main className="flex-1 flex flex-col bg-zinc-950/20 relative">
        {children}
      </main>
    </div>
  );
